...
Replace the temporary Gradio Space UI with the existing ChitraMaya Engine Studio UI so we maintain one frontend instead of two.

Current Space:

- Repo: `lablab-ai-amd-developer-hackathon/chitramaya-engine-video`
- URL: `https://huggingface.co/spaces/lablab-ai-amd-developer-hackathon/chitramaya-engine-video`
- Current SDK: Gradio
...
- SDK: Docker
- App: existing `dashboard/` React/Vite UI
- Runtime server: small HTTP server on port `7860`
- Backend: live ChitraMaya Engine API on the MI300X droplet
...
Hugging Face Space Docker app :7860
  ...
  v
ChitraMaya Engine backend droplet :8190
  |
  v
ComfyUI :8188 + MI300X
```
...
1. Serve static Studio build from `dashboard/dist`.
2. Proxy backend calls:
   - `/api/*` → `${CHITRAMAYA_API_URL}/api/*`
   - `/media/*` → `${CHITRAMAYA_API_URL}/media/*`
...
```bash
CHITRAMAYA_API_URL=http://134.199.200.202:8190
```
...
1. **Create a local Space Docker staging folder**
   ...
   - Copy current `dashboard/` source.
   - Add `Dockerfile`, `server.js`, root `package.json`, updated `README.md`.

2. **Verify local Docker build**

   ```bash
   docker build -t chitramaya-engine-space .
   docker run --rm -p 7860:7860 \
     -e CHITRAMAYA_API_URL=http://134.199.200.202:8190 \
     chitramaya-engine-space
   ```
...
This should replace the Gradio Space entirely and make the Hugging Face Space a hosted version of the real ChitraMaya Engine Studio UI.
