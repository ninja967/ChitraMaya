...
class AiToolkitRuntime(BaseModel):
    ...
    training_dir: str = "/root/chitramaya-training"
    runner: str = "/root/chitramaya-training/run-ai-toolkit.sh"
...
class Settings(BaseSettings):
    ...
    gpu_nodes_json: str | None = Field(default=None, validation_alias="CHITRAMAYA_GPU_NODES")
    comfy_nodes_json: str | None = Field(default=None, validation_alias="CHITRAMAYA_COMFY_NODES")
    ...
    def gpu_nodes(self) -> list[GpuNode]:
        ...
        if self.gpu_nodes_json:
            ...
            if not isinstance(parsed, list):
                raise ValueError("CHITRAMAYA_GPU_NODES must be a JSON array")
            raw_nodes = parsed
        elif self.comfy_nodes_json:
            parsed = json.loads(self.comfy_nodes_json)
            if not isinstance(parsed, list):
                raise ValueError("CHITRAMAYA_COMFY_NODES must be a JSON array")
