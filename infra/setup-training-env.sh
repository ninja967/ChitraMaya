...
# Run after infra/bootstrap.sh so ROCm/system basics are already present.
...
TRAINING_DIR="${TRAINING_DIR:-/root/chitramaya-training}"
...
# If this script is run from a cloned ChitraMaya repo, seed our checked-in config templates
...
# ChitraMaya AI Toolkit Training Workspace
...
- Datasets: `/root/chitramaya-training/datasets`
- Configs: `/root/chitramaya-training/config`
- Outputs/checkpoints: `/root/chitramaya-training/output`
- Sample control images: `/root/chitramaya-training/samples`

## Run a config

```bash
cd /root/ai-toolkit
/root/ai-toolkit-venv/bin/python run.py /root/chitramaya-training/config/<config>.yaml
```
...
#!/bin/bash
set -Eeuo pipefail
CONFIG_PATH="${1:?Usage: /root/chitramaya-training/run-ai-toolkit.sh /root/chitramaya-training/config/job.yaml}"
cd /root/ai-toolkit
...
echo "Run CLI:  $TRAINING_DIR/run-ai-toolkit.sh /root/chitramaya-training/config/job.yaml"
