...
APP_REPO_URL="${APP_REPO_URL:-https://github.com/chitramaya-ai/chitramaya.git}"
APP_DIR="${APP_DIR:-/root/chitramaya}"
CHITRAMAYA_OUTPUT_DIR="${CHITRAMAYA_OUTPUT_DIR:-/root/ComfyUI/output}"
...
# install infra/workflow assets. The durable API, database, Studio UI, and
# control plane live on the VPS.
echo "=== Cloning/updating ChitraMaya repo ==="
...
# NOTE: Studio frontend and ChitraMaya Engine API are hosted on the VPS, not on the
...
echo "Studio UI and ChitraMaya Engine API are hosted on the VPS."
echo "On the VPS, set COMFY_URL=http://<droplet-ip>:8188 in chitramaya-engine-api.service and restart it."
echo "Install FLUX.2 image stack: $APP_DIR/infra/setup-image-models.sh"
echo "Install Wan 2.2 video stack: $APP_DIR/infra/setup-video-models.sh"
echo "Install AI Toolkit training stack: $APP_DIR/infra/setup-training-env.sh"
