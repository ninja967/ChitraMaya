...
CHITRAMAYA_API_URL="${CHITRAMAYA_API_URL:-}"
...
IMAGE_STACK_TEST_PROMPT="${IMAGE_STACK_TEST_PROMPT:-realistic social media creator photo, confident person filming lifestyle content, modern dashboard setup, smartphone camera, soft natural light, polished Instagram aesthetic}"
IMAGE_STACK_TEST_CHECKPOINT="${IMAGE_STACK_TEST_CHECKPOINT:-latest}"

mkdir -p \
  "$COMFY_DIR/models/diffusion_models" \
  "$COMFY_DIR/models/loras/chitramaya-engine" \
...
LORA_OUTPUT_DIR="${CHITRAMAYA_LORA_OUTPUT_DIR:-}"
if [ -n "$LORA_OUTPUT_DIR" ] && [ -d "$LORA_OUTPUT_DIR" ]; then
    find "$LORA_OUTPUT_DIR" -maxdepth 1 -type f -name '*.safetensors' -print0 | while IFS= read -r -d '' lora; do
        ln -sf "$lora" "$COMFY_DIR/models/loras/chitramaya-engine/$(basename "$lora")"
    done
fi
...
if [ -n "$CHITRAMAYA_API_URL" ]; then
    curl -sS --max-time 10 "$CHITRAMAYA_API_URL/api/health"
fi

if [ "$RUN_IMAGE_STACK_TEST" = "1" ]; then
    if [ -z "$CHITRAMAYA_API_URL" ]; then
        echo "ERROR: RUN_IMAGE_STACK_TEST=1 requires CHITRAMAYA_API_URL pointing at the VPS control-plane API."
        exit 1
    fi
    curl -sS -X POST "$CHITRAMAYA_API_URL/api/lora-training/generate" \
