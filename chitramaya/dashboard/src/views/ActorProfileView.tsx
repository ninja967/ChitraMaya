...
export function ActorProfileView({ characterId, items, onOpen, onDelete, onGenerate }: ActorProfileViewProps) {
      ...
      <section className="space-y-4">
        ...
        {shown.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {shown.map((item) => (
              <AssetCard key={item.filename || item.url} item={item} onOpen={() => onOpen(item.url)} onDelete={onDelete} />
            ))}
          </div>
        )}
      </section>
