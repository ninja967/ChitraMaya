...
const apiTarget = normalizeApiUrl(process.env.CHITRAMAYA_API_URL || process.env.CHITRAMAYA_ENGINE_URL || "");
...
app.addHook("onRequest", async (request, reply) => {
  if (request.url.startsWith("/api/") || request.url === "/api" || request.url.startsWith("/media/") || request.url === "/media") {
    if (!backendProxy) {
      return reply.code(503).send({ detail: "CHITRAMAYA_API_URL is not configured" });
    }
