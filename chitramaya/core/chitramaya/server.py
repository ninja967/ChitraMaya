...
from .settings import ComfyNode, get_settings
...
async def agent_chat(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    lower = last_text.lower().strip()

    try:
        # Health / status queries
        if any(kw in lower for kw in ("health", "status", "online", "backend", "gpu")):
            tool_result = await health()
            return {"ok": True, "text": f"Backend is {'online' if tool_result.get('ok') else 'offline'}. ComfyUI nodes online: {tool_result.get('comfy_nodes_online', 0)}/{tool_result.get('comfy_nodes_total', 0)}.", "tool": "health", "data": tool_result}

        # Character listing
        if any(kw in lower for kw in ("character", "cast", "who can i use")):
            chars = await characters()
            names = [c.get("name", c.get("id")) for c in chars.get("characters", [])]
            return {"ok": True, "text": f"Available characters: {', '.join(names) if names else 'none yet'}.", "tool": "characters", "data": chars}

        # Training status
        if any(kw in lower for kw in ("training", "lora", "fine-tun", "finetun")):
            status_result = await lora_training_status()
            return {"ok": True, "text": f"LoRA training status: {status_result.status}. Progress: {status_result.progress_percent or 0}%.", "tool": "lora_status", "data": status_result.model_dump()}

        # Gallery / listing
        if any(kw in lower for kw in ("gallery", "listing", "show me", "what do i have")):
            media = await listing(limit=10)
            count = media.get("total", 0)
            return {"ok": True, "text": f"Gallery has {count} media items.", "tool": "listing", "data": media}

        # Image generation intent
        if any(kw in lower for kw in ("generate image", "make image", "create image", "photo of", "picture of", "portrait")):
            prompt_text = last_text
            for prefix in ("generate an image of", "generate image of", "make an image of", "create an image of", "make me a", "generate a"):
                if lower.startswith(prefix):
                    prompt_text = last_text[len(prefix):].strip()
                    break
            return {
                "ok": True,
        ...
        if any(kw in lower for kw in ("generate video", "make video", "create video", "animate", "clip of")):
            prompt_text = last_text
            return {
                "ok": True,
        ...
        if any(kw in lower for kw in ("project", "movie", "trailer", "short film", "storyboard", "put me in")):
            return {
                "ok": True,
                "text": "That sounds like a multi-shot project! I can draft a project with scenes and shots. Use the Projects tab in the sidebar to get started.",
                "tool": "project_intent",
            }

    except Exception as exc:  # noqa: BLE001
        return {"ok": False, "text": f"Tool execution failed: {exc}", "error": str(exc)}
