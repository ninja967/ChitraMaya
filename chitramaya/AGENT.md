...
| **Projects** | …pitches a *concept* — a story, a video idea, "put me in a power-suit launch scene", a beat sequence with multiple shots. | `/api/projects/...` |
...
```bash
export CHITRAMAYA_API_URL="http://<backend-host>:8190"
curl -sS "$CHITRAMAYA_API_URL/api/health"
```
...
```bash
curl -sS "$CHITRAMAYA_API_URL/api/characters"
```

For direct Studio generation, pass one character with the `character` shortcut:

```json
{"character":"maya_prototype","prompt":"dashboard portrait, dramatic light"}
```
...
{
  "characters": [
    {"id":"maya_prototype","role":"hero","lora_strength":1.0}
  ],
...
Do not claim ChitraMaya can train a LoRA from this skill alone. The current backend exposes LoRA training status/checkpoint reads, but no `/api/lora-training/start` route is implemented in the API code.
...
curl -sS -X POST "$CHITRAMAYA_API_URL/api/image/generate" \
  ...
  -d '{"character":"maya_prototype","prompt":"in an open-helmet power suit getting ready to take off"}'
```

## Text-to-video

```bash
curl -sS -X POST "$CHITRAMAYA_API_URL/api/video/generate" \
  -H "Content-Type: application/json" \
...
```bash
curl -sS -X POST "$CHITRAMAYA_API_URL/api/images/upload" -F "file=@/path/to/source.png"
curl -sS -X POST "$CHITRAMAYA_API_URL/api/video/generate" \
  -H "Content-Type: application/json" \
...
```bash
curl -sS "$CHITRAMAYA_API_URL/api/jobs/<prompt_id>"
```
...
1. **Hear the pitch.** Decide whether it is clearly multi-shot. "Make a trailer of me becoming a power-suit pilot" is a project. "Make me look like a power-suit pilot" is Studio. If unclear, ask: "Do you want one image/clip for fun, or a multi-shot stitched together into a legit final output (think TikTok video)?"
...
## Worked example: "Put me in a power-suit launch scene"

Your human says: *"Put me in a power-suit launch scene — suit-up, launch, rooftop landing, the whole thing."*

You decide: this is a project because it asks for a sequence of movie beats. Cast `maya_prototype`. Aspect `9:16`. ~30s. About 3 scenes, 1–3 shots each. Title: *Suit Up*.
...
{
  "title": "Suit Up",
  "description": "MayaPrototype suits up in a workshop and steps out into the rain.",
  "aspect_ratio": "9:16",
  "duration_seconds": 30,
  "characters": ["maya_prototype"]
}
...
Then your shots — for scene 2: `shot 1: wide of MayaPrototype on the assembly platform`, `shot 2: medium of chest plate locking in`, `shot 3: close on the helmet snapping shut, eyes glow blue`. Each gets an `image_prompt` and `motion_prompt`.
...
## LoRA Training (API-driven)

You can now start LoRA training from the API. The flow is:

### 1. Upload a dataset

```bash
curl -sS -X POST "$CHITRAMAYA_API_URL/api/datasets/upload" \
  -F "name=my-character" \
  -F "file=@character-dataset.zip"
```

The zip should contain images (`.png`, `.jpg`, etc.) and matching `.txt` caption files.

### 2. List available datasets

```bash
curl -sS "$CHITRAMAYA_API_URL/api/datasets"
```

### 3. Start training

```bash
curl -sS -X POST "$CHITRAMAYA_API_URL/api/lora-training/start" \
  -H "Content-Type: application/json" \
  -d '{
    "job_name": "my_character_lora",
    "trigger_word": "MyCharacter",
    "dataset_path": "/root/chitramaya-training/datasets/my-character",
    "base_model": "flux2",
    "steps": 1800
  }'
```

Supported `base_model` values: `"flux2"` (FLUX.2 image LoRA) or `"wan22_i2v"` (Wan 2.2 I2V character LoRA).

### 4. Monitor training

```bash
curl -sS "$CHITRAMAYA_API_URL/api/lora-training/status"
curl -sS "$CHITRAMAYA_API_URL/api/lora-training/checkpoints"
```

### 5. Stop training (if needed)

```bash
curl -sS -X POST "$CHITRAMAYA_API_URL/api/lora-training/stop"
```

Once training completes, checkpoints are automatically available for image generation via the character's LoRA binding.

---

## Don'ts

- Don't open the ComfyUI browser UI. ComfyUI is the headless execution engine — you talk to ChitraMaya's API, not Comfy directly.
