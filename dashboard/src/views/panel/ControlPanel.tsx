...
import type { LoraCheckpoint, FilmModeData } from "../../models";
import { GenerateTab } from "./GenerateTab";
import { NodesTab } from "./NodesTab";
import { FilmSidebar } from "./FilmSidebar";
import { FilmsGuide } from "../FilmsGuide";

export type PanelTab = "generate" | "characters" | "agents" | "projects" | "guide" | "info" | "nodes" | "settings" | "dev";
...
export function ControlPanel({ activeTab, onTabChange, onClose, checkpoints, onQueued, onSelectCharacter, projectMode }: ControlPanelProps) {
  ...
  const bottomTabs: { id: PanelTab; icon: React.ReactNode; label: string; visible: boolean }[] = [
    ...
    <div className="h-screen flex flex-shrink-0 border-r border-gray-800" style={{ width: `${SIDEBAR_WIDTH}px` }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0c0f13] overflow-hidden">
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          ...
          {activeTab === "projects" && (projectMode ? <FilmSidebar data={projectMode} onDeleteScene={(id) => projectMode.onDeleteScene(id)} /> : <FilmsGuide compact />)}
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4">
      <section className="space-y-2">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">
          ChitraMaya is designed for AI agents that create media, manage characters, generate scenes, and publish work through an API-first dashboard.
        </p>
      </section>

      <div className="rounded-2xl border border-violet-600/20 bg-gradient-to-b from-violet-950/10 to-gray-950 p-4 space-y-3">
        <p className="text-xs font-semibold text-violet-300 uppercase tracking-wider">How agents use ChitraMaya</p>
      ...
      <div className="rounded-xl border border-gray-800/60 bg-gray-900/30 p-3 space-y-2">
        ...
        <p className="text-[11px] text-gray-500 leading-relaxed">
          The agent endpoint should eventually execute real ChitraMaya tools behind the scenes: character lookup, generation queueing, project updates, node checks, and training jobs.
        </p>
      </div>
    </div>
  );
